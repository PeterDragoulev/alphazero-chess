...
AB_DEPTH = 4          # alpha-beta (material) search depth for the auto-play foe
NN_SIMS = 200         # MCTS sims/move for the neural net in auto-play
MAX_AUTO_PLIES = 300  # adjudicate an auto-play game as a draw beyond this
LIGHT_SQUARE = "#f0d9b5"
DARK_SQUARE = "#b58863"
PIECE_ICONS = {
    chess.WHITE: {
...
class ChessUI:
    def __init__(self, root: tk.Tk) -> None:
        ...
        # auto-play (engine vs engine) state
        self.auto_play = False
        self.nn_color = chess.WHITE          # which side the neural net plays
        self.nn_wins = self.nn_draws = self.nn_losses = 0
        ...
        self.auto_button = tk.Button(
            root, text=f"Auto-Play: NN vs αβ(d{AB_DEPTH})",
            command=self.toggle_auto_play)
        self.auto_button.grid(row=6, column=0, padx=10, pady=(0, 10), sticky="w")

        self.score_label = tk.Label(
            root, text=f"NN vs αβ d{AB_DEPTH}:  +0 =0 -0", anchor="w")
        self.score_label.grid(row=6, column=1, columnspan=3, padx=5, sticky="w")
    ...
    def reset_game(self) -> None:
        self.auto_play = False
        self.auto_button.configure(text=f"Auto-Play: NN vs αβ(d{AB_DEPTH})")
        self.board.reset()
        self.last_move = None
        self.selected_square = None
        self.valid_moves = set()
    ...
    # -- auto-play: neural net vs alpha-beta(d4), engine vs engine -----------

    def toggle_auto_play(self) -> None:
        if self.auto_play:                       # currently running -> stop
            self.auto_play = False
            self.auto_button.configure(text=f"Auto-Play: NN vs αβ(d{AB_DEPTH})")
            self.set_status("Auto-play stopped.")
            return
        if _model is None:
            self.set_status("No neural net loaded — train/pretrain first.")
            return
        self.auto_play = True
        self.player_vs_bot = False
        self.mode_var.set(False)
        self.auto_button.configure(text="Stop Auto-Play")
        self._start_auto_game()

    def _start_auto_game(self) -> None:
        self.board.reset()
        self.last_move = None
        self.selected_square = None
        self.valid_moves = set()
        side = "White" if self.nn_color == chess.WHITE else "Black"
        self.set_status(f"Auto-play: neural net is {side} vs alpha-beta d{AB_DEPTH}.")
        self.update_ui()
    ...
    def auto_step(self) -> None:
        ...
        if self.board.is_game_over(claim_draw=True) \
                or len(self.board.move_stack) >= MAX_AUTO_PLIES:
            self._record_auto_result()
            return
        if self.board.turn == self.nn_color:
            move = choose_move(self.board, simulations=NN_SIMS)
        else:
            move = _material_move(self.board, depth=AB_DEPTH)
        if move is None:
            self._record_auto_result()
            return
        self.board.push(move)
    ...
    def _record_auto_result(self) -> None:
        outcome = self.board.outcome(claim_draw=True)
        if outcome is None or outcome.winner is None:
            self.nn_draws += 1
            verdict = "draw"
        elif outcome.winner == self.nn_color:
            self.nn_wins += 1
            verdict = "neural net wins"
        else:
            self.nn_losses += 1
            verdict = "alpha-beta wins"
        self.score_label.configure(
            text=f"NN vs αβ d{AB_DEPTH}:  "
                 f"+{self.nn_wins} ={self.nn_draws} -{self.nn_losses}")
        self.set_status(f"Game over — {verdict}. Next game starting...")
        self.nn_color = not self.nn_color        # alternate colors for fairness
        if self.auto_play:
            self.root.after(1800, self._start_auto_game)
